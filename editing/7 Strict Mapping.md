...
到现在为止，我们已经使用了`map`,`nmap`,`vmap`和`imap`来建立按键映射，用以节省时间。这个方法确实很有效，但是这个方法也存在问题。
...
###递归映射
运行下面的命令:
<pre><code>:nmap dd O<esc>jddk</code></pre>
你可能会认为这个命令会把`dd`映射成以下几步：
...
##副作用
并不只是我们所学到的`*map`命令会有递归的风险，并且我们安装的其他的插件也会有这样的风险。
